...
async function analyzeLink1() {
  console.log('='.repeat(50));
  console.log('LINK.1 상세 분석 (Step by Step)');
  console.log('='.repeat(50));
  ...
  try {
    ...
    console.log('\n상품 페이지로 이동...');
    await page.goto('https://labgolf.com/products/link-1-custom', { waitUntil: 'domcontentloaded' });
    ...
    // ACCRA 선택
    const accraSelect = await page.evaluate(() => {
      const labels = document.querySelectorAll('label.option-avis-swatch-value-label');
      for (const label of labels) {
        if (label.offsetParent !== null && label.textContent?.includes('ACCRA')) {
          label.click();
          return { success: true };
        }
      }
    ...
    console.log(accraSelect.success ? '  ACCRA 선택됨' : '  ACCRA 선택 실패');
    ...
    await page.evaluate(() => {
      ...
      for (const select of selects) {
        if (select.offsetParent !== null) {
          const option = Array.from(select.options).find(opt => opt.value.includes('28') || opt.textContent?.includes('28'));
          if (option) {
    ...
    // Step 2 옵션 다시 확인
    console.log('\n=== Shaft 선택 후 Step 2 옵션 ===\n');
    const step2AfterShaft = await page.evaluate(() => {
      const results = [];
      const optionTitles = document.querySelectorAll('.avp-option-title');
      for (const title of optionTitles) {
        const container = title.closest('.avp-option');
    ...
    step2AfterShaft.forEach(o => console.log(`  - ${o}`));
